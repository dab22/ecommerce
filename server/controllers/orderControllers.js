...
//         if(!orders)return res.status(404).json({success: false, message: "no orders to show"});
...
//     if(req.role !== ROLES.admin)return res.status(403).json({success: false, message: "not authorised"});
...
//         return res.status(200).json({success: true, data: orders, totalPages: Math.ceil(count/limit), currentPage: Number(page)});
...
// module.exports = {getOrdersByUserId, getAllOrders, updateOrderStatus, getMetrics};
...
const getMetrics = async (req, res) => {
  ...
  try {
    const start = isNaN(new Date(startDate))
      ? new Date(new Date().setMonth(new Date().getMonth() - 1))
      : new Date(startDate);
    const end = isNaN(new Date(endDate)) ? new Date() : new Date(endDate);
    ...
    // Six months product stats by category
    const sixMonthsAgo = new Date(new Date().setMonth(new Date().getMonth() - 6));
    const sixMonthsOrders = await Order.find({
      createdAt: { $gte: sixMonthsAgo },
    }).populate({
