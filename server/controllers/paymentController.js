...
const generatePayment = async(req, res)=>{
    ...
    try{
        ...
        instance.orders.create(options, async(err, order)=>{
            ...
            return res.status(200).json({success: true, data:{...order, name: user.name}})
        });
...
const verifyPayment = async(req, res)=>{
    ...
    if(!validatedPayment) return res.status(400).json({success: false, message: "Payment verification failed"});
